...
<div class="container-fluid">
    ...
    <div class="row">
    	<div class="col">
			<button type="button" class="btn btn-warning create-btn" onclick="toshiftCreate();">＋作成</button>
    	</div>
    </div>
	@component('components.navbar')
	@slot('nav')
	<a class="nav-item nav-link bg-light attendance-nav" onclick="toAttendance();">勤怠</a>
    <a class="nav-item nav-link active bg-info">シフト</a>
		...
		if( data !== null ){
			...
			for(let i=0; i<date_shift.length; i++){
				if( date_info == date_shift[i] ){
					memberHTML[i] = '<p>' + (shift_name[i]+'　　').slice(0,3) + '&nbsp;' + go_time[i] + '~' + display_out[i] + '</p><br>';
...
function toshiftfixAdmin(){
	$(document).on('click','.fix-btn',function(){
		window.location.href="/admin/shift-fix";
	});
}
